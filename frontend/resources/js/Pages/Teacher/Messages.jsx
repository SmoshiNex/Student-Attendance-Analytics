...
import {
  ...
  Wifi, WifiOff, ChevronLeft, ChevronDown, X,
  Users, BookOpen, Paperclip, Image, FileText, Download, ZoomIn,
} from "lucide-react"
...
function AttachmentBubble({ msg, isOwn }) {
  ...
  // PDF or generic file
  const Icon = type === "pdf" ? FileText : FileText
  return (
    <a
      href={url}
      download={name}
...
export default function TeacherMessages() {
  ...
  useEffect(() => {
    if (!teacher) return
    socket.emit("register", { user_type: "teacher", user_id: teacher.id })
  ...
  const loadInbox = useCallback(() => {
    if (!teacher) return
    axios.get(messagesApiUrl("inbox", { user_type: "teacher", user_id: teacher.id }), { withCredentials: true })
  ...
  const loadClasses = useCallback(() => {
    ...
    axios.get(teacherClassApiUrl(), { withCredentials: true })
      ...
      .catch(console.error).finally(() => setLoadingClasses(false))
  }, [teacher])

  const loadClassStudents = useCallback((classId) => {
    if (!teacher) return
    setLoadingStudents(true); setClassStudents([])
    axios.get(teacherClassApiUrl({ action: "students", id: classId }), { withCredentials: true })
  ...
  const loadConversation = useCallback((thread) => {
    ...
    axios.get(messagesApiUrl("conversation", {
      sender_type: "teacher", sender_id: teacher.id,
    ...
    axios.post(messagesApiUrl("mark_read"), {
      reader_type: "teacher", reader_id: teacher.id,
  ...
  const sendMessage = async () => {
    if ((!draft.trim() && !pendingFile) || !activeThread || !teacher || sending) return
    setSending(true)

    let attachmentUrl = null, attachmentType = null, attachmentName = null

    // 1. Upload file first if pending
    if (pendingFile) {
  ...
  const handleDraftChange = (e) => {
    ...
    if (!activeThread || !teacher) return
    socket.emit("typing", { sender_type: "teacher", sender_id: teacher.id, receiver_type: activeThread.partner_type, receiver_id: activeThread.partner_id, is_typing: true })
    clearTimeout(typingTimerRef.current)
    typingTimerRef.current = setTimeout(() =>
      socket.emit("typing", { sender_type: "teacher", sender_id: teacher.id, receiver_type: activeThread.partner_type, receiver_id: activeThread.partner_id, is_typing: false }), 1500)
  }
  ...
  const openNewChat = () => {
    setShowNewChat(true); setSelectedClass(null); setClassStudents([]); setStudentSearch(""); setMobilePanel("chat")
    if (classes.length === 0) loadClasses()
  }
  const selectClass = (cls) => { setSelectedClass(cls); setStudentSearch(""); loadClassStudents(cls.id) }
  const startChatWithStudent = (student) => {
    const thread = { partner_type: "student", partner_id: student.id, partner_name: `${student.first_name} ${student.last_name}`.trim(), partner_student_id: student.student_id }
    setActiveThread(thread); setShowNewChat(false); setSelectedClass(null); setClassStudents([]); loadConversation(thread)
  }
  const goBackToSidebar = () => { setMobilePanel("sidebar"); setShowNewChat(false); setSelectedClass(null); setClassStudents([]) }

  const filteredThreads  = threads.filter((t) => t.partner_name?.toLowerCase().includes(searchQuery.toLowerCase()))
  const filteredStudents = classStudents.filter((s) => {
    const q = studentSearch.toLowerCase()
    return `${s.first_name} ${s.last_name}`.toLowerCase().includes(q) || (s.student_id || "").toLowerCase().includes(q)
  })
    ...
    <div className="min-h-screen bg-gray-100 teacher-shell">
      ...
      <main className="max-w-7xl mx-auto py-0 sm:py-6 px-0 sm:px-6 lg:px-8">
        <div className="bg-white sm:rounded-xl shadow-sm overflow-hidden flex h-[calc(100vh-4rem)] sm:h-[calc(100vh-8rem)] border-0 sm:border border-gray-200">
          ...
          <div className={`flex flex-col border-r border-gray-200 shrink-0 w-full md:w-80 ${mobilePanel === "sidebar" ? "flex" : "hidden"} md:flex`}>
            <div className="px-4 py-3 border-b border-gray-100 bg-gray-50 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={openNewChat} className="text-xs bg-gray-900 text-white rounded-lg px-2.5 py-1 hover:bg-gray-700 transition-colors font-medium">+ New</button>
              </div>
            </div>
              ...
              {loadingInbox ? <div className="py-10 text-center text-sm text-gray-400">Loading...</div>
                : filteredThreads.length === 0 ? (
                  <div className="py-10 text-center">
                    ...
                    <button onClick={openNewChat} className="mt-2 text-xs text-blue-600 hover:underline">Start a new conversation</button>
                  </div>
                  ...
                  return (
                    <button key={`${thread.partner_type}_${thread.partner_id}`} onClick={() => openThread(thread)}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] mt-0.5 text-gray-400">Student{thread.partner_student_id ? ` • ${thread.partner_student_id}` : ""}</p>
                      </div>
                    </button>
                  )
                })}
          ...
          {/* Chat / New Message panel */}
          <div className={`flex-1 flex flex-col min-w-0 ${mobilePanel === "chat" ? "flex" : "hidden"} md:flex`}>
            {showNewChat ? (
              <div className="flex-1 flex flex-col">
                <div className="px-4 py-3 border-b border-gray-100 bg-gray-50 flex items-center gap-2">
                  <button onClick={goBackToSidebar} className="md:hidden text-gray-500 hover:text-gray-800 p-1 -ml-1 rounded-lg"><ChevronLeft className="w-5 h-5" /></button>
                  <Users className="w-4 h-4 text-gray-700" />
                  <h2 className="text-sm font-semibold text-gray-900 flex-1">New Message</h2>
                  <button onClick={goBackToSidebar} className="hidden md:block text-gray-400 hover:text-gray-700"><X className="w-4 h-4" /></button>
                </div>
                <div className="flex-1 overflow-y-auto p-5">
                  {!selectedClass ? (
                    <div>
                      <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-1.5"><BookOpen className="w-3.5 h-3.5" /> Step 1 — Select a Class</p>
                      {loadingClasses ? <p className="text-sm text-gray-400 py-6 text-center">Loading classes…</p>
                        : classes.length === 0 ? <p className="text-sm text-gray-400 py-6 text-center">No classes found.</p>
                        : <div className="flex flex-col gap-2">{classes.map((cls) => (
                          <button key={cls.id} onClick={() => selectClass(cls)} className="w-full text-left rounded-xl border border-gray-200 px-4 py-3 hover:border-gray-900 hover:bg-gray-50 transition-all">
                            <p className="text-sm font-semibold text-gray-900">{cls.class_name || cls.subject_name}</p>
                            <p className="text-xs text-gray-400 mt-0.5">{cls.class_code}{cls.schedule ? ` • ${cls.schedule}` : ""}{cls.room ? ` • ${cls.room}` : ""}</p>
                          </button>
                        ))}</div>}
                    </div>
                  ) : (
                    <div>
                      <div className="flex items-center gap-2 mb-3">
                        <button onClick={() => { setSelectedClass(null); setClassStudents([]) }} className="text-gray-400 hover:text-gray-700"><ChevronDown className="w-4 h-4 rotate-90" /></button>
                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider flex items-center gap-1.5"><Users className="w-3.5 h-3.5" /> Step 2 — Select a Student</p>
                      </div>
                      <div className="mb-3 flex items-center gap-2 bg-gray-100 rounded-lg px-3 py-2">
                        ...
                        <span className="text-xs font-medium text-gray-700 truncate">{selectedClass.class_name || selectedClass.subject_name} ({selectedClass.class_code})</span>
                      </div>
                      <div className="relative mb-3">
                        <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" />
                        <input type="text" placeholder="Search student name or ID…" value={studentSearch} onChange={(e) => setStudentSearch(e.target.value)}
                          className="w-full pl-8 pr-3 py-2 text-sm rounded-lg bg-gray-100 border-0 focus:ring-1 focus:ring-gray-300 outline-none" />
                      </div>
                      {loadingStudents ? <p className="text-sm text-gray-400 py-6 text-center">Loading students…</p>
                        : filteredStudents.length === 0 ? <p className="text-sm text-gray-400 py-6 text-center">{studentSearch ? "No students match." : "No students enrolled."}</p>
                        : <div className="flex flex-col gap-1">{filteredStudents.map((s) => {
                          const fullName = `${s.first_name} ${s.last_name}`.trim()
                          return (
                            <button key={s.id} onClick={() => startChatWithStudent(s)} className="w-full text-left rounded-xl px-3 py-2.5 hover:bg-gray-100 transition-colors flex items-center gap-3">
                              <div className="h-8 w-8 rounded-full bg-gray-900 text-white text-xs font-semibold flex items-center justify-center shrink-0">{initials(fullName)}</div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center px-6">
                ...
                <p className="text-sm text-gray-400">Choose a student from the left, or tap <strong className="text-gray-600">+ New</strong> to start a chat.</p>
              </div>
            ) : (
              <>
                {/* Chat Header */}
                <div className="px-4 py-3 border-b border-gray-100 bg-gray-50 flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-gray-400">Student{activeThread.partner_student_id ? ` • ${activeThread.partner_student_id}` : ""}</p>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-gray-50">
                  {loadingMsgs ? <div className="text-center text-sm text-gray-400 py-10">Loading messages...</div>
                    ...
                    : messages.map((msg) => {
                      const isOwn = msg.sender_type === "teacher" && Number(msg.sender_id) === Number(teacher?.id)
                      return (
