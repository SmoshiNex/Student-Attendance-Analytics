...
export default function EditClassModal({ isOpen, onClose, classItem }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.patch(
                teacherClassApiUrl({ id: classItem.id }),
                data,
            ...
            onClose();
            setErrorModal({ open: true, message: error?.response?.data?.message || "Failed to update class." });
        } finally {
            setProcessing(false);
        }
    };
                ...
                <DialogContent>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            />
                            {errors.class_code && <p className="text-sm text-red-500">{errors.class_code}</p>}
                        </div>
