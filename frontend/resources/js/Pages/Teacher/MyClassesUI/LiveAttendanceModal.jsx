...
export default function LiveAttendanceModal({ isOpen, onClose, classData }) {
    ...
    const fetchLiveData = async (sessionId) => {
        try {
            ...
            if (!sessionStartRef.current && updatedSession?.started_at) {
                const serverTime = response.data.server_time ?? updatedSession.started_at;
    ...
    const endSession = async () => {
        try {
            await axios.post(attendanceApiUrl("close_session", { session_id: session.id }), {}, { withCredentials: true });
            setSession(null);
            setRecords([]);
                    ...
                    {!session ? (
                        ...
                        <div className="space-y-4 py-6">
                            ...
                            <p className="text-sm text-gray-600">
                                Enter time allowed for students to check in as
                                PRESENT:
                            </p>
                    ...
                    <div className="fixed inset-0 flex items-center justify-center z-[10000] pointer-events-none">
                        <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4 shadow-2xl pointer-events-auto">
                            ...
                            <p className="text-gray-600 mb-6">
                                Are you sure you want to end this attendance
                                session? Students will no longer be able to
                                check in.
                            </p>
