...
export function ConfirmModal({ open, title = "Are you sure?", message, confirmLabel = "Confirm", cancelLabel = "Cancel", onConfirm, onCancel, danger = false }) {
    if (!open) return null;
    return (
        <Overlay onClose={onCancel}>
            <div className="bg-white rounded-2xl shadow-2xl p-6">
                <div className="flex items-center gap-3 mb-4">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 ${danger ? "bg-red-100" : "bg-yellow-100"}`}>
                ...
                <div className="flex gap-3 mt-2">
                    ...
                    <button
                        onClick={onConfirm}
                        className={`flex-1 py-2.5 text-white rounded-lg font-medium transition-colors ${danger ? "bg-red-600 hover:bg-red-700" : "bg-black hover:bg-gray-800"}`}
                    >
                        {confirmLabel}
                    </button>
                </div>
